...
\begin{frame}{The Big Picture: GEMM Dataflow on the SM}
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[eng, minimum width=2.95cm, minimum height=1.35cm] (cuda) at (4.65,0.2) {};
      \node[font=\scriptsize] at (4.65,0.62) {CUDA cores};
      \node[draw, thick, fill=blue!10, font=\tiny, align=center,
            minimum width=1.55cm, minimum height=0.44cm] (regs) at (4.65,-0.06) {registers};
      ...
      \draw[flow] (cuda.west) to[out=180,in=-60] (smem.south east);
      % store back
      ...
      \node[steplab] at (1.7,0.6)    {\textbf{3}\,drain \& stage};
      \node[sstep]   at (-3.55,0.82) {\textbf{4}\,store};
    \end{tikzpicture}
  \end{center}

  \vspace{0.1cm}
  {\footnotesize Green ellipses are \textbf{engines}, rectangles \textbf{storage};
  step 3 is the CUDA-core epilogue --- drain TMEM into registers, then stage to SMEM.}
\end{frame}
