// Runnable companion for Chapter 11 — hoist descriptor builds above
// the mbarrier wait (latency hiding).
//
// Identical to ch09 except for the MMA warp's K-loop structure: the
// K_MMAS A and B descriptor builds are moved from *after* the
// mbarrier_wait_phase(tile_ready) call to *before* it.  The
// descriptor math only depends on `slot`, which is computed at the
// top of each K-iter — it does not depend on whether the TMA bytes
// have landed.  Hoisting the math above the wait lets the scheduler
// overlap it with the wait's spin loop, and lets the MMA issuer
// fire all K_MMAS back-to-back below with no descriptor-build
// stalls interleaved.
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    // ── MMA warp — only CTA 0 issues; cta_group::2 result lands in both CTAs' TMEM
        ...
        for (int k = 0; k < num_k_iters; k++) {
            ...
            // ── Hoist descriptor builds ABOVE the mbar wait ──────────
            // These addresses only depend on `slot`, not on the
            // tile-ready signal, so they can be computed before we
            // know the data has landed.  The compiler / scheduler can
            // then overlap the descriptor arithmetic with the wait's
            // spin loop instead of running them serially after the
            // wait returns.  Pre-built descriptors also let the MMA
            // issuer fire all K_MMAS back-to-back below with no
            // descriptor-build stalls interleaved.
            uint64_t a_desc[K_MMAS], b_desc[K_MMAS];
            #pragma unroll
            for (int kk = 0; kk < K_MMAS; kk++) {
                a_desc[kk] = make_desc(
                    A_base(slot) + kk * MMA_K * BF16_BYTES);
                b_desc[kk] = make_desc_K_major(
...
void matmul_hoistdesc_ns##NS_##_gsm##GSM_(                                               \
