...
//
// Builds on ch07's kernel (K-major B, multi-stage, warp-spec,
// multi-CTA grid, coalesced epilogue) and adds 2-CTA cluster
// cooperation:
//
//   * __cluster_dims__(2, 1, 1) on the kernel.  Each pair of CTAs
//     runs as a cluster.
//   * %cluster_ctarank built-in → cta_rank in {0, 1}.
//   * tcgen05.alloc.cta_group::2 + tcgen05.mma.cta_group::2 →
//     M per MMA doubles to 2*BM = 256.  Only cta_rank==0 issues MMA.
//   * tile_ready mbar count = 2 (both CTAs' TMA must arrive); each
//     CTA routes its expect_tx to CTA 0's mbar via the &0xFEFFFFFFu
//     mask.
//   * tcgen05.commit ... multicast::cluster fires both CTAs' mma_done
//     mbars from CTA 0's commit issuer.
//   * barrier.cluster.arrive/wait replaces __syncthreads after init
//     so peer mbar inits are visible cluster-wide before TMA touches
//     them.
//
// NS is a template parameter so main.py can compile NS=2..7 variants
// and sweep them — see the README for the perf table.
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    const int bid_m      = cluster_id / grid_n;
    const int bid_n      = cluster_id % grid_n;
    const int off_m_cluster = bid_m * (CTA_GROUP * BM);    // cluster M-base
    const int off_n         = bid_n * BN;                  // shared by both CTAs
...
// ── Launchers — one per NS so main.py can sweep ─────────────────────
#define MAKE_LAUNCHER(NS_)                                                            \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1) __launch_bounds__(THREADS, 1) \
void matmul_cluster_ns##NS_(                                                          \
    const __grid_constant__ CUtensorMap A_tmap,                                       \
    const __grid_constant__ CUtensorMap B_tmap,                                       \
    __nv_bfloat16* C_ptr, int M, int N, int K)                                        \
{                                                                                      \
    matmul_cluster_impl<NS_>(&A_tmap, &B_tmap, C_ptr, M, N, K);                       \
}
