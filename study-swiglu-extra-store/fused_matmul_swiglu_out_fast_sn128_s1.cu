...
constexpr int STORE_N          = 128;                 // TMA-store chunk width
constexpr int TMA_STORE_STAGES = 1;                  // double-buffered store SMEM
...
#define EPI_TMEM_EMPTY_ARRIVE(buf)  do { uint32_t _e = ((uint32_t)__cvta_generic_to_shared(&tmem_empty[buf])) & 0xFEFFFFFFu; mbarrier_arrive_no_tx_cluster(_e); } while (0)
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                // high K — so it's a sweep knob, not always-on.
                {
                    ...
                    static_assert(STORE_N == 64 || STORE_N == 128, "pipelined TMA store assumes STORE_N=64 or 128");
                    static_assert(NUM_CHUNKS == 2 || NUM_CHUNKS == 4, "swiglu-out assumes BN=256 with STORE_N=64 or 128");
