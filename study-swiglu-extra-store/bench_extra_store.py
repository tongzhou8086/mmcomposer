...
def patch_extra_store(src: str, mode: str) -> str:
    ...
    ``swiglu-out-fast-precompute-d`` computes/packs the final D output before
    waiting for the TMA-store buffer, to test whether one-stage TMA is losing
    from source-level wait placement rather than inherent queue depth.
    ``swiglu-out-fast-overlap-wait-d`` starts the elected TMA wait before D
    math, allowing non-waiting epilogue warps to compute D while the wait is in
    flight, then synchronizes before writing the shared TMA buffer.
    ...
    if mode == "swiglu-out-fast-precompute-d":
        return patch_swiglu_out_store(src, fast_math=True, precompute_d=True)
    if mode == "swiglu-out-fast-overlap-wait-d":
        return patch_swiglu_out_store(src, fast_math=True, overlap_wait_d=True)
...
def patch_swiglu_out_store(src: str, *, fast_math: bool, precompute_d: bool = False,
                           overlap_wait_d: bool = False) -> str:
    ...
    if precompute_d or overlap_wait_d:
        old_d = f"""                        wait_for_store_buffer();
                        #pragma unroll
                        for (int n = 0; n < LOADS_PER_WARP; n++) {{
                            __nv_bfloat162 pk[4];
                            #pragma unroll
...
"""
        ...
        new_d = f"""{wait_prefix}                        __nv_bfloat162 d_pk[LOADS_PER_WARP][4];
                        #pragma unroll
                        for (int n = 0; n < LOADS_PER_WARP; n++) {{
                            #pragma unroll
                            for (int i = 0; i < 4; i++) {{
                                ...
                                d_pk[n][i] = __floats2bfloat162_rn(v0, v1);
                            }}
                        }}
...
{wait_suffix.rstrip()}
                        ...
                        for (int n = 0; n < LOADS_PER_WARP; n++) {{
                            ...
                            __nv_bfloat16* write_ptr =
                                C_store + store_stage * BM * STORE_N + my_row * STORE_N + swizzled_n * 8;
                            *reinterpret_cast<int4*>(write_ptr) = *reinterpret_cast<int4*>(d_pk[n]);
                        }}
                        issue_tma_store(D_tmap_ptr, EPI_OUT_COL_BASE / 2 + chunk * STORE_N);"""
        if old_d not in new:
            raise RuntimeError("could not find D compute block for swiglu-out precompute")
        new = new.replace(old_d, new_d, 1)
...
def render_compile(tier: dict, k: dict, arch: str) -> pathlib.Path:
    # The fused epilogue patch was written against the canonical two-stage
    # TMA-store source, whose rotation anchor is `store_stage ^= 1`.  After
    # TMA_STORE_STAGES became a generator knob, stage-1 renders specialize that
    # line away.  Render a stage-2 base for source patching, then specialize the
    # patched study copy to the requested stage count below.
    render_k = dict(k)
    render_k["tma_store_stages"] = 2
    base_src_path = gd.render_to_dir(tier, render_k)
    src = base_src_path.read_text()
    ...
    src_path = gd.SCRATCH / gd.tag_for(tier, k) / "kernel.cu"
    src_path.parent.mkdir(parents=True, exist_ok=True)
    src_path.write_text(src)
    if k.get("extra_store_mode") in (
        "swiglu-half", "swiglu-out", "swiglu-out-fast",
        "swiglu-out-fast-precompute-d", "swiglu-out-fast-overlap-wait-d",
        "swiglu-out-fast-dual-b",
    ):
        mode_name = k["extra_store_mode"].replace("-", "_")
        ns_suffix = "" if k.get("ns") == CONFIG["ns"] else f"_ns{k['ns']}"
        stage_path = HERE / f"fused_matmul_{mode_name}{ns_suffix}_s{k['tma_store_stages']}.cu"
        stage_path.write_text(src)
        if (k.get("extra_store_mode") == "swiglu-out-fast"
                and int(k["tma_store_stages"]) == 2
                and k.get("ns") == CONFIG["ns"]):
    ...
    try:
        ...
        for sh in shapes:
            ...
            c_tmap = gd.rt.encode_tensor_map(
                ...
                box_dim=[64, k["bm"]], element_strides=[1, 1],
                swizzle=gd.rt.TMA_SWIZZLE_128B,
            )
            ...
            if mode != "base":
                if mode in (
                    ...
                    "swiglu-out-fast", "swiglu-out-fast-precompute-d",
                    "swiglu-out-fast-overlap-wait-d", "swiglu-out-fast-dual-b",
                ):
                    d_tensor = sh["D_half"]
                    d_width = n // 2
                elif mode == "extra-full":
...
def main() -> int:
    ...
    ap.add_argument("--ns", type=int, default=CONFIG["ns"],
                    help="override K-loop SMEM ring stages for the study config")
    ap.add_argument("--gsm", type=int, default=CONFIG["gsm"],
                    help="override GROUP_SIZE_M for the study config")
    ap.add_argument("--nw", type=int, default=CONFIG["nw"],
                    help="override NUM_WARPS for the study config")
    ...
    for variant in variants:
        for stages in stages_list:
            ...
            print(f"\n# variant={variant} store_n=64 stages={stages} "
                  f"shared={shared} B ({shared / 1024:.1f} KiB)", flush=True)
            if shared > max_smem:
                print(f"SKIP variant={variant} store_n=64 stages={stages}: "
            ...
            for sh in shapes:
                ...
                print(
                    f"variant={variant} store_n=64 stages={stages} shape={key} "
