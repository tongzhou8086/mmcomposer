"""Self-contained cuda-python plumbing for mmcomposer host scripts.

Hides the per-call `(err, *rest)` tuple-unpacking, the nvcc compile
dance, and the cuLaunchKernel argument-packing.  mmcomposer prepends
this preamble to a tier launcher to produce a single standalone host
script — so the generated file runs with just torch + cuda-python +
nvcc, no sibling modules required.
"""
