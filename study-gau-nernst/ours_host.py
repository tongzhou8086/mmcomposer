...
Config: BM=128 BN=256 BK=64 NS=4 GROUP_SIZE_M=8 NUM_WARPS=4 TMA_STORE=0 PERSISTENT=1 EPILOGUE_OVERLAP=1 EPILOGUE_SPLIT=1
...
mod, fns = compile_kernel(os.path.join(HERE, "ours_kernel.cu"),
...
# ── Benchmark vs cuBLAS, Triton do_bench 500/500 (matches study-gau-nernst/bench.py) ──
import sys
...
for spec in shapes:
    ...
    ours_ms = triton.testing.do_bench(
        ...
        warmup=500, rep=500)
    cublas_ms = triton.testing.do_bench(lambda: torch.mm(A, B), warmup=500, rep=500)
