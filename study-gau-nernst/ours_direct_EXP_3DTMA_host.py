...
mod, fns = compile_kernel(os.path.join(HERE, "ours_direct_EXP_3DTMA_kernel.cu"),
...
# ── Reproduce the bug: EPILOGUE_DIRECT + EPILOGUE_OVERLAP works at 1 tile/cluster
# but faults (CUDA_ERROR_LAUNCH_FAILED) once a cluster handles >= 2 tiles.
#   2048x2048x768 ->  64 clusters < ~74 persistent slots -> ~1 tile/cluster (OK)
#   4096x4608x768 -> 288 clusters                        -> ~4 tiles/cluster (FAULTS)
grid_slots = max(1, NUM_SMS // CTA_GROUP)
for (M, N, K) in [(2048, 2048, 768), (4096, 4608, 768), (32768, 4608, 768)]:
    ...
    try:
        ...
        print(f"  launched OK, rel_err={rel:.2e}  ({'CORRECT' if rel < 5e-2 else 'WRONG'})")
        if (M, N, K) == (32768, 4608, 768):
            us = time_kernel_us(
            ...
            print(f"  ours   : {us:8.1f} us   {tflops:7.0f} TFLOPS   ({tflops / cub_tflops * 100:.0f}% of cuBLAS)")
