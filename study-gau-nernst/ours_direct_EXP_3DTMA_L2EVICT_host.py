...
Tier: Tier 3 — + 2-CTA cluster MMA
Config: BM=128 BN=256 BK=64 NS=4 GROUP_SIZE_M=8 NUM_WARPS=4 TMA_STORE=0 PERSISTENT=1 EPILOGUE_OVERLAP=1 EPILOGUE_SPLIT=0 EPILOGUE_DIRECT=1 TCGEN05_LD_WIDTH=16 EXP_6W=1 L1_NO_ALLOC=1 EXP_3DTMA=1
...
ELEM_BYTES   = 2
# EXP_6W mirrors gau-nernst's warp layout: NUM_WARPS epilogue warps plus
# one TMA warp and one MMA warp, with no two-warp padding gap.
THREADS      = (NUM_WARPS + 2) * 32 if EPILOGUE_OVERLAP else NUM_WARPS * 32
...
EPI_BYTES    = 0 if EPILOGUE_DIRECT else BM * EPI_LD * ELEM_BYTES
...
mod_base, fns_base = compile_kernel(os.path.join(HERE, "ours_direct_EXP_3DTMA_kernel.cu"),
                                    device, kernels=["matmul_cluster"])
kernel_base = fns_base["matmul_cluster"]

mod_l2evict, fns_l2evict = compile_kernel(
    os.path.join(HERE, "ours_direct_EXP_3DTMA_L2EVICT_kernel.cu"),
    device, kernels=["matmul_cluster"])
kernel_l2evict = fns_l2evict["matmul_cluster"]

ext.load("gn_v7_paired",
         sources=[os.path.join(HERE, "matmul_v7.cu"),
...
for kernel in (kernel_base, kernel_l2evict):
...
def setup(M, N, K):
    ...
    B_storage = torch.randn(N, K, dtype=torch.bfloat16, device="cuda")
    B = B_storage.T
    C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")

    A_tmap = encode_tensor_map(
        dtype=TMA_BFLOAT16, rank=3, gptr=A.data_ptr(),
        global_dim=[64, M, K // 64], global_strides=[K * ELEM_BYTES, 128],
        box_dim=[64, BM, BK // 64], element_strides=[1, 1, 1],
        swizzle=TMA_SWIZZLE_128B)
    B_tmap = encode_tensor_map(
        dtype=TMA_BFLOAT16, rank=3, gptr=B_storage.data_ptr(),
        global_dim=[64, N, K // 64], global_strides=[K * ELEM_BYTES, 128],
        box_dim=[64, BN_LOCAL, BK // 64], element_strides=[1, 1, 1],
        swizzle=TMA_SWIZZLE_128B)
...
grid_slots = max(1, NUM_SMS // CTA_GROUP)
shapes = sys.argv[1:] or ["32768x4608x768"]
for spec in shapes:
    M, N, K = (int(x) for x in spec.split("x")) if "x" in spec else (int(spec),) * 3
    n_clusters = (M // (CTA_GROUP * BM)) * (N // BN)
    tiles_per  = -(-n_clusters // grid_slots)
    A, B, C, grid, args = setup(M, N, K)
    print(f"\n=== {M}x{N}x{K}  ({n_clusters} clusters, ~{tiles_per} tile(s)/cluster) ===")
    try:
        C_ref = (A.float() @ B.float()).to(torch.bfloat16)
        flops = 2.0 * M * N * K
        cub_us = time_kernel_us(lambda: torch.mm(A, B), warmup_ms=1000, rep_ms=1000)
        cub_tflops = flops / (cub_us * 1e-6) / 1e12
        print(f"  cuBLAS         : {cub_us:8.1f} us   {cub_tflops:7.0f} TFLOPS")

        for label, kernel in (
            ("baseline L1", kernel_base),
            ("L1 + L2 EF ", kernel_l2evict),
        ):
            C.zero_()
            launch(kernel, grid=grid, block=(THREADS, 1, 1), shared=SHARED_BYTES, args=args)
            cu(driver.cuCtxSynchronize())
            rel = (C.float() - C_ref.float()).abs().max().item() / C_ref.float().abs().max().item()
            us = time_kernel_us(
                lambda: launch(kernel, grid=grid, block=(THREADS, 1, 1),
                               shared=SHARED_BYTES, args=args, sync=False),
                warmup_ms=1000, rep_ms=1000)
            tflops = flops / (us * 1e-6) / 1e12
            ok = "OK" if rel < 5e-2 else "FAIL"
            print(f"  {label}    : {us:8.1f} us   {tflops:7.0f} TFLOPS   "
                  f"({tflops / cub_tflops * 100:.0f}% of cuBLAS, {ok}, rel_err={rel:.2e})")

        out = ops.matmul_v7c(A, B)
        rel = (out.float() - C_ref.float()).abs().max().item() / C_ref.float().abs().max().item()
        v7c_us = time_kernel_us(lambda: ops.matmul_v7c(A, B), warmup_ms=1000, rep_ms=1000)
        v7c_tflops = flops / (v7c_us * 1e-6) / 1e12
        ok = "OK" if rel < 5e-2 else "FAIL"
        print(f"  gau v7c        : {v7c_us:8.1f} us   {v7c_tflops:7.0f} TFLOPS   "
              f"({v7c_tflops / cub_tflops * 100:.0f}% of cuBLAS, {ok}, rel_err={rel:.2e})")
    except Exception as e:
        print(f"  *** FAILED: {type(e).__name__}: {e}")
        break   # context is poisoned after a launch failure


try:
    cu(driver.cuModuleUnload(mod_l2evict))
    cu(driver.cuModuleUnload(mod_base))
