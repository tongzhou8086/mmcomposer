...
Config: BM=128 BN=256 BK=64 NS=5 GROUP_SIZE_M=1 NUM_WARPS=4 TMA_STORE=0 PERSISTENT=1 EPILOGUE_OVERLAP=1 EPILOGUE_SPLIT=1
...
mod, fns = compile_kernel(os.path.join(HERE, "autotune_4096_best_kernel.cu"),
...
for (M, N, K) in [(4096, 4096, 4096)]:
    ...
    print(f"     BM={BM} BN={BN} BK={BK} NS={NS} GSM={GROUP_SIZE_M} NW={NUM_WARPS} "
          ...
          f"{us:7.1f} us/call   "
          f"{tf:6.1f} TFLOPS")
    print(f"     cuBLAS torch.mm contiguous-B: {cub_us:7.1f} us/call   "
          f"{cub_tf:6.1f} TFLOPS   MMComposer={tf / cub_tf * 100:.1f}%")
    print()
