...
Config: BM=128 BN=256 BK=64 NS=4 GROUP_SIZE_M=8 NUM_WARPS=4 TMA_STORE=0 PERSISTENT=1 EPILOGUE_OVERLAP=1 EPILOGUE_SPLIT=1 A_3D_TMA=1 B_3D_TMA=1 L1_NO_ALLOC=1
...
mod, fns = compile_kernel(os.path.join(HERE, "ours_staged_EXP_AB3D_L1NOALLOC_kernel.cu"),
...
# ── Benchmark vs cuBLAS, Triton do_bench 1000/1000 ───────────────────
import sys
...
for spec in shapes:
    ...
    ours_ms = triton.testing.do_bench(
        lambda: launch(kernel, grid=grid, block=(THREADS, 1, 1),
                       shared=SHARED_BYTES, args=args, sync=False),
        warmup=1000, rep=1000)
    cublas_ms = triton.testing.do_bench(lambda: torch.mm(A, B), warmup=1000, rep=1000)
    ours_tf = flops / (ours_ms * 1e-3) / 1e12
    cub_tf  = flops / (cublas_ms * 1e-3) / 1e12

    print(f"\n=== {M}x{N}x{K} ===  ({ok}, rel_err={rel:.2e})")
    print(f"  cuBLAS : {cublas_ms*1000:8.1f} us   {cub_tf:7.0f} TFLOPS")
    print(f"  ours   : {ours_ms*1000:8.1f} us   {ours_tf:7.0f} TFLOPS   "
          f"({ours_tf/cub_tf*100:.0f}% of cuBLAS)")
    print(f"           BM={BM} BN={BN} BK={BK} NS={NS} GSM={GROUP_SIZE_M} NW={NUM_WARPS} "
          f"TWO_CTA={TWO_CTA} PERSISTENT={PERSISTENT} "
          f"OVERLAP={EPILOGUE_OVERLAP} SPLIT={EPILOGUE_SPLIT} TMA_STORE={TMA_STORE}")
