...
constexpr int TCGEN05_LD_WIDTH = 16;  // TMEM->reg epilogue load width: 8 or 16 (32-bit elems per lane)
...
constexpr int LAUNCH_THREADS = (NUM_WARPS + 2) * WARP_SIZE;
...
__device__ __forceinline__ void tma_3d_load_g2(
    uint32_t smem_dst, const void* tmap, int x, int y, int z, uint32_t mbar
) {
    asm volatile(
        "cp.async.bulk.tensor.3d.shared::cluster.global.mbarrier::complete_tx::bytes.cta_group::2 "
        "[%0], [%1, {%2, %3, %4}], [%5];"
        :: "r"(smem_dst), "l"(tmap), "r"(x), "r"(y), "r"(z), "r"(mbar) : "memory");
}
...
// The 3D-TMA layout matches gau-nernst v7, so do not set the K-major B bit.
    ...
    uint32_t idesc, bool first_k_tile)
{
    ...
    for (int kk = 0; kk < K_MMAS; kk++) {
        ...
        const uint64_t b_desc = make_desc(b_base_slot + kk * MMA_K * BF16_BYTES);
...
__device__ __forceinline__ void matmul_cluster_impl(
...

    {
        ...
        // Direct mode skips SMEM staging entirely, so no C_sh at all — the
        // whole dynamic SMEM is the K-ring.
        ...
        if (warp_id == NUM_WARPS && elect_sync()) {
            uint32_t ph[NS] = {};
            long gk = 0;
            for (int ti = 0; ti < num_my; ti++) {
                ...
                for (int k = 0; k < num_k; k++) {
                    ...
                    tma_3d_load_g2(A_base(slot), A_tmap, 0, local_m, k, ready_mb_cta0);
                    tma_3d_load_g2(B_base(slot), B_tmap, 0, local_n, k, ready_mb_cta0);
...
#define EPI_TMEM_EMPTY_ARRIVE(buf)  do { uint32_t _e = ((uint32_t)__cvta_generic_to_shared(&tmem_empty[buf])) & 0xFEFFFFFFu; mbarrier_arrive_no_tx_cluster(_e); } while (0)
            ...
            for (int ti = 0; ti < num_my; ti++) {
                ...
                // EPILOGUE_DIRECT skips SMEM staging entirely: TMEM -> regs ->
                // GMEM with no C_sh round-trip and no per-pass bar.syncs.  The
                // GMEM stores are strided (each lane writes a different row), but
                // when the epilogue is exposed (low K, short main loop) the lean
                // path stalls less than the coalesced staged store, and it frees
                // the whole epilogue SMEM for a deeper K-ring.  (Borrowed from
                // gau-nernst's v7 sm100 kernel.)
                {
                    // Monolithic ld -> wait -> cvt -> st.global per WIDTH-column
                    // chunk (gau-nernst v7 pattern).  One asm block keeps the
                    // whole chunk register-resident and stops the compiler from
                    // reordering the global store against the async tcgen05.ld.
                    // Lane `lane` owns GMEM row (EPI_OUT_ROW + my_row).
                    #pragma unroll
                    for (int n = col_base; n < col_base + COLS_PER_WARP; n += LDW) {
                        __nv_bfloat16* gptr = &C_ptr[(EPI_OUT_ROW + my_row) * N + EPI_OUT_COL_BASE + n];
                        asm volatile(
                            ...
                            "st.relaxed.cta.global.L1::no_allocate.L2::evict_first.v8.b32 "
                            "[%0], {b0,b1,b2,b3,b4,b5,b6,b7};\n\t}"
                            :: "l"(gptr), "r"(trow + (uint32_t)n) : "memory");
                    }
                    // Barrier so every epilogue warp has finished reading TMEM
                    // (tcgen05.ld) before ew==0 hands the buffer back to MMA.
